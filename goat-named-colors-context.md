...
.repomixignore
css/main.css
Goat Named Colors.html
images/copy.svg
images/favicon.svg
js/GoatColorToolbox.js
js/main.js
README.md
repomix.config.json
```
...
.swatch-container.light-swatch .copy-icon {
    stroke: #000000;
}

.swatch-container.dark-swatch .copy-icon {
    stroke: #ffffff;
}
...
<body>
    ...
    <div class="page-container"> <!-- Main page wrapper for color list -->
        <ul class="color-list" id="colorList">
            <!-- Color items will be injected here by JavaScript -->
        </ul>
    </div>

    <div id="copyNotification" class="copy-notification">Copied!</div>

    <script src="js\GoatColorToolbox.js"></script>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const svgIconMarkup = `
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="copy-icon">
                    <rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect>
                    <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path>
                </svg>`;
    ...
    function isColorLight(colorInstance) {
        ...
        const luminance = colorInstance.getRelativeLuminance();
    ...
    function renderColorList(colorsToRender) {
        ...
        colorsToRender.forEach(colorData => {
            ...
            const isLight = isColorLight(instance);
            swatchContainer.classList.add(isLight ? 'light-swatch' : 'dark-swatch');
            ...
            swatchContainer.innerHTML += svgIconMarkup;

            const nameSpan = document.createElement('span');
            nameSpan.className = 'color-name';
            nameSpan.textContent = name;

            listItem.appendChild(swatchContainer);
            listItem.appendChild(nameSpan);

            listItem.addEventListener('click', () => {
                navigator.clipboard.writeText(name).then(() => {
                    copyNotificationElement.textContent = `Copied "${name}"!`;
                    ...
                    copyNotificationElement.textContent = 'Copy failed!';
                    copyNotificationElement.style.backgroundColor = '#E53935';
                    copyNotificationElement.classList.remove('hide');
                    copyNotificationElement.classList.add('show');
                    clearTimeout(notificationTimeout);
                    notificationTimeout = setTimeout(() => {
                        copyNotificationElement.classList.remove('show');
                        copyNotificationElement.classList.add('hide');
                        copyNotificationElement.style.backgroundColor = '#4CAF50';
                    }, 2000);
                });
            });
    ...
    function sortAndRender() {
        ...
        if (sortBy === 'name') {
            ...
            processedColors.sort((a, b) => {
                // Handle achromatic colors (those with effectiveHue === -1)
                if (a.effectiveHue === -1 && b.effectiveHue === -1) { // Both are achromatic
                    return a.lightness - b.lightness; // Sort by lightness (black to white)
                }
                if (a.effectiveHue === -1) return -1; // a is achromatic, b is chromatic, a comes first
                if (b.effectiveHue === -1) return 1;  // b is achromatic, a is chromatic, b comes first

                // Both are chromatic
                if (a.hue !== b.hue) {
                    return a.hue - b.hue; // Primary sort by actual hue
                }
                // If actual hues are the same, sort by lightness
                if (a.lightness !== b.lightness) {
                    return a.lightness - b.lightness;
                }
                // If lightness is also the same, sort by saturation
    ...
    const rawColorNames = Object.keys(GoatColor.cssNamedColors);
    processedColors = rawColorNames.map(name => {
        const instance = GoatColor(name);
        let actualHue = 0, saturation = 0, lightness = 0;
        let effectiveSortHue; // This will be used for sorting

        if (instance.isValid()) {
            const hsl = instance.toHsl();
            actualHue = isNaN(hsl.h) ? 0 : hsl.h;
            saturation = hsl.s;
            lightness = hsl.l;

            if (saturation === 0) { // Achromatic (black, white, grays, transparent's base)
                effectiveSortHue = -1; // Special sort key to group achromatic colors
            } else {
                effectiveSortHue = actualHue;
            }
        } else {
            // Should not happen with GoatColor.cssNamedColors, but as a fallback
            effectiveSortHue = -1; // Treat invalid as achromatic for sorting
        }

        return {
            name: name,
            instance: instance,
            hue: actualHue,          // Store the actual hue
            saturation: saturation,  // Store saturation
            lightness: lightness,    // Store lightness
            effectiveHue: effectiveSortHue // Hue value used for primary sorting logic
        };
    });
...
`![CSS Color Lister Screenshot](path/to/your/screenshot.png)`
...
- [GoatColorToolbox.js](./GoatColorToolbox.js) (included)
- SVG (for icons)
...
```json
{
  ...
  "output": {
    "filePath": "goat-named-colors-context.md",
    "style": "markdown",
    "parsableStyle": false,
    "fileSummary": true,
